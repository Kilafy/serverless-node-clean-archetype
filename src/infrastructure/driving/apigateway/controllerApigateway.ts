...
= (serviceUseCase: BasicUseCase, zodSchema: z.Schema) => async (event: APIGatewayEvent, context: unknown) => {
	try {
		console.log(context);
		...
		if (!validateResult.success) {
			return getHttpResponse.badRequest(validateResult.message, validateResult.errors);
		}
